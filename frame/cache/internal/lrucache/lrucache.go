...
func (c *Cache) GetPtr(key string) (interface{}, bool) {
	return c.getPtr(key)
}
...
func (c *Cache) SetPtr(key string, value interface{}) {
	c.mu.Lock()
...
func (c *Cache) addNewV2(key string, value interface{}) {
	newEntry := &entry{
		key:      key,
		valuePtr: value,
		addedAt:  time.Now(),
